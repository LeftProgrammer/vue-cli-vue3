...
import { updateModelConfig } from "../api";
//测量距离
...
// 设置视角
function setViewPoint(that, sceneInfo) {
  that.$alert(`确定设置当前视角为默认视角`, `默认视角`, {
    confirmButtonText: "确定",
    callback: (action) => {
      const { heading, pitch, position } = that.api.Camera.getViewPort();
      const params = {
        modelId: that.bimId,
        defaultViewpoint: JSON.stringify({ heading, pitch, position }),
      };
      updateModelConfig(params).then(() => {
...
function clearTrace(that) {
  ...
  document.getElementById(`cesiumContainer${that.bimId}`).style.cursor =
    "default";
}

function MeasurementType(type, that, sceneInfo) {
  switch (type.id) {
    case "DefaultViewPoint": //默认视角
      setViewPoint(that, sceneInfo);
      break;
