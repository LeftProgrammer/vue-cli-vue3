...
export default {
  ...
  methods: {
    ...
    setGanttConfig() {
      ...
      gantt.config.drag_move = false; //拖动移动
      ...
      var colContent = function (task) {
        return `<span  class='pointer text-041ACB'  onclick="clickGridButton('${
          ...
          <span class='pointer text-F53F3F ml-8' style="display:${
            task.hasChild == 1 ? "none" : ""
          }"   onclick="clickGridButton('${task.id}','delete')">删除</span>
          `;
      };
        ...
        },
        {
          ...
          template: (task) => {
            const type = this.taskTypes.find(
              (item) => item.dictCode === task.taskType
            );
            return type ? type.dictName : task.taskType;
          },
        },
        {
          name: "units",
