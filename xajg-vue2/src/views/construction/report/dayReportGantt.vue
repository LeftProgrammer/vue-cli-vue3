...
export default {
  ...
  methods: {
    ...
    restGanttConfig(data, flag) {
      if (!flag) gantt.init('gantt_day_report')
      gantt.parse({
        data: data
      })
      gantt.render()
    ...
    setGanttConfig(flag) {
      ...
      console.log('🚀 ~ setGanttConfig ~  gantt.config:', gantt.config)
      ...
      let gantt_width = document.getElementById('gantt_day_report').offsetWidth
      console.log('🚀 ~ setGanttConfig ~ gantt_width:', gantt_width)
      gantt.config.layout = {
        css: 'gantt_container',
        cols: [
          {
      ...
      var colContent = function (task) {
        return `<span  style="display:${
          task.hasChild ? 'none' : ''
        }" class='pointer text-041ACB '   onclick="clickGridButton('${
          task.id + ''
        }','report')">日报</span>
          `
      }
      let actualNumConetnt = function (task) {
        return `<span  class='pointer text-041ACB ' onclick="clickGridButton('${
          task.id + ''
        }','history')">${task.actualNum || ''}</span>
          `
      }
