<template>
  <div class="page-archival-catalogue-permission">
    <TreeTableLayout
      :page="searchData"
              ...
              >
                <template #default="{ row }">
                  <div class="table-btn">
                    ...
                    <el-button
                      type="text"
                      @click="deleteAdvanceStorePermission(row)"
                      >删除</el-button
                    >
                  </div>
                </template>
...
import {
  ...
  getSelectAdvanceStorePermission,
  addAdvanceStorePermission,
  deleteAdvanceStorePermission,
  updateAdvanceStorePermission,
  getAdvancePermissionList,
...
export default {
  name: "ArchivalCataloguePermission",
  components: {
    TreeTableLayout,
    UserSelectInput,
    DragLine,
  ...
  methods: {
    ...
    async getTree() {
      ...
      if (res?.length > 0) {
        ...
        this.$nextTick(() => {
          ...
          this.getSelectAdvanceStorePermission();
        });
      } else {
        this.treeData = [];
      }
    ...
    async getArchiveCatalogueManagerTree() {
      try {
        const { data, success, message } =
          await getArchiveCatalogueManagerTree();
        if (!success) {
    ...
    // 查表格数据
    async getSelectAdvanceStorePermission() {
      this.tableData = [];
      try {
        const { data, success, message } =
          await getSelectAdvanceStorePermission(this.searchData);
    ...
    async getAdvancePermissionList() {
      try {
        const { data, success, message } = await getAdvancePermissionList();
        if (!success) {
    ...
    async addAdvanceStorePermission() {
      if (!this.selectionNode) {
        this.$message.error("请先选择左侧节点！");
        return;
      }
      try {
        const { data, success, message } = await addAdvanceStorePermission(
    ...
    handelPageSizeChange(page) {
      ...
      this.getSelectAdvanceStorePermission(this.selectionNode.id);
    },
    // 切换页数
    ...
    async updateAdvanceStorePermission() {
      try {
        const { data, success, message } = await updateAdvanceStorePermission(
          this.addDialogFormData
        );
        if (!success) {
    ...
    async handelDialogConfirm() {
      ...
      if (this.addDialogFormData.id) {
        // 编辑
        res = await this.updateAdvanceStorePermission();
      } else {
        // 添加
        res = await this.addAdvanceStorePermission();
      }
      if (res) {
        this.showDialog = false;
        this.getSelectAdvanceStorePermission();
    ...
    // 删除数据
    async deleteAdvanceStorePermission(row) {
      try {
        await this.beforeDelete();
        const { data, success, message } = await deleteAdvanceStorePermission({
          id: row.id,
        });
        if (!success) {
...
.page-archival-catalogue-permission {
  height: 100%;
  overflow: hidden;
  color: #fff;
  .content {
