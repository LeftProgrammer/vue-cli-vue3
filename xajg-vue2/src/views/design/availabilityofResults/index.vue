<template>
  <div class="page-leave">
    <treeTableLayout
      :page="pageParams" :title="title" :show-export-btn="true" @query="getTableData"
      @pageSizeChange="handleSizeChange" @pageCurrentChange="handleCurrentChange" @reset="reset"
      @initExportParams="initExportParams"
    >
      <!-- :showExportBtn="!(fromPage == 'hydroStandardForm')" -->
      <template slot="form">
        <el-form ref="form" :model="query" :inline="true">
          <el-form-item :label="fromPage == 'hydroStandardForm' ? '文件名称:' : '成果名称:'" size="mini">
            <el-input v-model="query.name" placeholder="请输入名称" clearable />
          </el-form-item>
          <el-form-item :label="fromPage == 'hydroStandardForm' ? '文件编号:' : '成果编号:'" size="mini">
            <el-input v-model="query.code" placeholder="请输入编号" clearable />
          </el-form-item>
          <el-form-item :label="fromPage == 'hydroStandardForm' ? '文件名称:' : '成果类型:'" size="mini">
            <el-select
              v-model="query.resultsType" placeholder="请选择成果类型" clearable
              @visible-change="$visibleChange($event, 'el-popper')"
            >
              <el-option v-for="item in sjcgOptionsList" :key="item.id" :label="item.dictName" :value="item.dictCode" />
            </el-select>
          </el-form-item>
          <el-form-item label="专业:" size="mini">
            <el-select
              ref="flowStateRef" v-model="query.profession" placeholder="请选择" clearable
              @visible-change="$visibleChange($event, 'el-popper')"
            >
              <el-option
                v-for="item in designProfessionOptions" :key="item.id" :label="item.dictName"
                :value="item.dictCode"
              />
            </el-select>
          </el-form-item>
          <el-form-item v-if="!(fromPage == 'hydroStandardForm')" label="发起日期:" size="mini">
            <el-date-picker
              v-model="startTimeAndEndTime" type="daterange" value-format="yyyy-MM-dd"
              style="width: 230px"
            />
          </el-form-item>
          <el-form-item v-if="!(fromPage == 'hydroStandardForm')" label="流程状态:" size="mini">
            <el-select
              ref="flowStateRef" v-model="query.flowStatus" style="width: 100%" placeholder="请选择" clearable
              @visible-change="$visibleChange($event, 'el-popper')"
            >
              <el-option
                v-for="item in flowStatusOptions" :key="item.id" :label="item.dictName"
                :value="item.dictCode"
              />
            </el-select>
          </el-form-item>
          <el-form-item v-if="isSDResult" label="所属标段:" size="mini" prop="sectionId">
            <el-select
              v-model="query.sectionId" class="w-100pre" placeholder="请选择" clearable
              @visible-change="$visibleChange($event, 'el-popper')"
            >
              <el-option v-for="item in sectionOptions" :key="item.id" :label="item.name" :value="item.id" />
            </el-select>
          </el-form-item>
          <el-form-item v-if="fromPage == 'hydroStandardForm'" label="成果类型:" size="mini">
            <el-select
              ref="flowStateRef" v-model="query.resultsType" style="width: 100%" placeholder="请选择" clearable
              @visible-change="$visibleChange($event, 'el-popper')"
            >
              <el-option v-for="item in sjcgOptionsList" :key="item.id" :label="item.dictName" :value="item.dictCode" />
            </el-select>
      ...
      <template slot="opratebtns">
        <el-button v-if="!(fromPage == 'hydroStandardForm')" type="primary" icon="el-icon-plus" @click="add">
          新增
        </el-button>
      </template>
      <template slot="table">
        <el-table
          v-if="showTable" ref="multipleTable" :data="tableData" height="100%" border :row-key="(row) => row.id"
          :highlight-current-row="!multiple" @header-dragend="handleHeaderDragEnd"
          @selection-change="handleSlectCurrentChange" @current-change="handleCurrent"
        >
          <!-- fromPage=='hydroStandardForm' 需要展示成果类型 -->
          <el-table-column
            v-if="multiple && fromPage == 'hydroStandardForm'" type="selection"
            :width="$calculateWidth(80)" align="center"
          />
          <el-table-column
            v-if="fromPage == 'hydroStandardForm'" label="成果类型" prop="title" align="left"
          ...
          <el-table-column
            v-if="!(fromPage == 'hydroStandardForm')" label="序号" type="index"
            :width="$calculateWidth(54)" align="center" :index="getIndex"
          />

          <el-table-column :label="fromPage == 'hydroStandardForm' ? '文件名称' : '成果名称'" prop="name" align="left">
            <template slot-scope="{ row }">
              <!-- <el-tooltip
              ...
              > -->
              <el-link
                type="primary" class="text-ellipsis" @click="
                  edit(
                    row,
                    'fine',
          ...
          <el-table-column
            :label="fromPage == 'hydroStandardForm' ? '文件编号' : '成果编号'" prop="code" align="left"
          ...
          <el-table-column
            v-if="!(fromPage == 'hydroStandardForm')" label="成果类型" prop="resultsType" align="center"
            :excel.dictType="sjcgCodeList" :width="$calculateWidth(120)"
          >
            <template slot-scope="{ row }">
              <!-- <el-tooltip
          ...
          <el-table-column
            label="专业" prop="profession" align="center" excel.dictType="DESIGN_PROFESSION"
          ...
          <el-table-column
            v-if="!(fromPage == 'hydroStandardForm')" label="所属标段" prop="sectionId"
            excel.handler="excelHandleCovert" excel.args="section" align="center" :width="$calculateWidth(120)"
          >
            <template slot-scope="{ row }">
              <div class="text-ellipsis">
          ...
          <!--          </el-table-column>-->
          <el-table-column
            v-if="!(fromPage == 'hydroStandardForm')" label="发起人" prop="createUser" align="center"
            excel.handler="excelHandleCovert" excel.args="user" :width="$calculateWidth(120)"
          >
            <template slot-scope="{ row }"> {{ row.createUsername }}</template>
          </el-table-column>
          <el-table-column
            v-if="!(fromPage == 'hydroStandardForm')" label="发起时间" prop="createDate" align="center"
            sortable excel.dateFormat="yyyy-MM-dd HH:mm" :width="$calculateWidth(180)"
          >
            <template slot-scope="{ row }">
              {{
                row.createDate
          ...
          <el-table-column
            v-if="fromPage == 'hydroStandardForm'" label="供应日期" prop="actualDate" align="center" sortable
            excel.dateFormat="yyyy-MM-dd" :width="$calculateWidth(180)"
          >
            <template slot-scope="{ row }">
              {{
          ...
          <el-table-column
            v-if="!(fromPage == 'hydroStandardForm')" label="流程状态" prop="flowStatus" align="center"
            :width="$calculateWidth(120)" excel.readConverterExp="0=未提交,1=流转中,2=已完成"
          >
          ...
          <el-table-column
            v-if="!(fromPage == 'hydroStandardForm')" label="当前节点" prop="flowName" align="center"
            :width="$calculateWidth(180)"
          >
            <template slot-scope="{ row }">
          ...
          <el-table-column
            v-if="!(fromPage == 'hydroStandardForm')" fixed="right" label="操作" align="center"
            width="200"
          >
            <template #default="{ row }">
              <div class=" ">
                <!-- :btns="
                  isSDResult
                    ? ['deal', 'press', 'delete', 'uploadOa']
                    : ['deal', 'press', 'delete']
                " -->
                <flow-button
                  :row="row" :flow-name="flowName" @click="edit" @delete="deletedata"
    ...
    <flow-dialog :visible="flowShow" :flow-info="flowInfo" @childEvt="childEvtHandle" @closed="flowShow = false" />
  </div>
</template>
