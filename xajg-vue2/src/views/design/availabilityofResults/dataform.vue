    ...
    <el-row>
      ...
      >
        ...
        <el-col
          ...
          :span="12"
        >
          <el-form-item label="监理总监" prop="supervisorDirector">
            <user
        ...
        <!-- <el-col :span="24">
          <el-col :span="6">
            <el-form-item label="业主接收部门" prop="receivingDepartment1">
              <el-select :disabled="isDisabled" @visible-change="$visibleChange($event, 'el-popper')" class="w-100pre"
                v-model="formData.receivingDepartment1" @change="changeDepartName">
                <el-option v-for="item in receivingDepartment" :label="item.departName" :key="item.departId"
                  :value="item.departId"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="" prop="receivingDepartment2" label-width="20px">
              <el-select :disabled="isDisabled" @visible-change="$visibleChange($event, 'el-popper')" class="w-100pre"
                v-model="formData.receivingDepartment2">
                <el-option v-for="item in secretaryOption(formData.receivingDepartment1)" :label="item.realName"
                  :key="item.userId" :value="item.userId"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-col> -->
...
export default {
  ...
  data() {
    return {
      ...
      unitList: [], //源数据
      formDataRules: {
        resultsType: [{ required: true, message: "请选择成果类型", trigger: "change" }],
        ...
        sectionId: [{ required: true, message: "请选择所属标段", trigger: "change" }],
        supervisorDirector: [
          { required: false, message: "请选监理总监", trigger: "change" }
        ],
        receivingDepartment1: [
        ...
        isChange: [{ required: true, message: "请选择是否变更图纸", trigger: "change" }],
        changeProcess: [
        ...
        handlerSignature: [{ required: true, message: "请上传经办人手写签名", trigger: "change" }],
        constructionUnit: [
          {
  ...
  computed: {
    isFflczq() {
      ...
      if (
        ...
        !this.formData.needDistribution
      ) {
        return true;
      } else {
        return false;
    ...
    // 何时显示附件
    fflczq() {
      console.log(" this.flowAuths?.fflczq", this.formData, this.flowAuths);
      if (this.formData?.parentId) {
        return false;
      }
      if (
        ...
        this.formData?.attachmentRecipient?.includes(this.$getStorage("userInfo").userId)
      ) {
        return true;
      } else {
        return false;
    ...
    isDisabled() {
      ...
      return false;
      // return this.readonly || this.formData.parentId ? true : false
    },
    ...
    pageType() {
      ...
      return (fileList) => {
        ...
        return (
          !this.formData.resultsType || !fileList.includes(this.formData.resultsType)
        );
      };
    }
  },
    ...
    "formData.resultsType": {
      handler(newVal) {
        ...
        this.$nextTick(() => {
          ...
          const notInAllProps = allProps.filter((prop) => !nowProps.includes(prop));
  ...
  methods: {
    ...
    async beforeSend() {
      let extData = {};
      // 内部审批人
      // extData["supervisorDirector"] = {
      //   type: "user",
      //   value: this.formData.supervisorDirector,
      // };
      // 施工单位内部审核人
      // extData["sgzyr"] = {
      //   type: "user",
    ...
    async getReceivingDepartment() {
      ...
      ownDepart.forEach((item) => {
        // 判断  部门不可能重复 直接赋值为[]
        // 从文秘数据中找出当前部门下的文秘
        let Secretarys = SecretaryList.filter((x) => x.departId === item.departId);
    ...
    async sure() {
      ...
      if (currentRow) {
        ...
        if (this.component == "MonthlyPlan") {
          ...
          this.$set(this.formData.designSupply, "profession", currentRow.profession);
    ...
    async save(origin, callback, event) {
      ...
      if (this.postMessageData?.btnKey == "return" || (!this.sgdwxs && this.showNtko)) {
        // 附件取消掉签章
    ...
    sectionIdFormatter(row, column) {
      ...
      for (var i = 0; i < this.sectionOptions.length; i++) {
        console.log("fdsfdsfsdfdfsdffsdf");
        ...
        if (row) {
          ...
          if (row.reviewCommentsFileList) {
            ...
            if (!Array.isArray(row.reviewCommentsFileList)) {
              row.reviewCommentsFileList = JSON.parse(row.reviewCommentsFileList);
            }
          } else {
...
//
//::v-deep .sectionSelect .el-select__tags{
//  top:30px;
//}
//
//::v-deep .sectionSelect .el-select__tags>span{
//  display: flex;
//}
//
//::v-deep .sectionSelect .el-tag{
//  max-width: 100px;
//  overflow: hidden;
