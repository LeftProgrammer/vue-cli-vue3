      ...
      <el-row :gutter="gutter">
        ...
        <el-col :span="24">
          <el-form-item label="施工标段" prop="section" align="center">
            ...
            >
              <el-option
                v-for="item in sectionFixedOptions"
                :key="item.id"
                :label="item.name"
                :value="item.id"
              />
        ...
        <el-col :span="24">
          <el-form-item label="记录人员" prop="recordingPersonnel"
        ...
        <el-col :span="24" class="grid-cell">
          <el-form-item label="水文与气象情况" prop="hydrometeorology">
            <el-input
              v-model="formData.hydrometeorology"
              type="textarea"
              clearable
              rows="3"
              maxlength="500"
        ...
        <el-col :span="24" class="grid-cell">
          <el-form-item label="安全监理情况" prop="safetySupervision">
            <el-input
              type="textarea"
              v-model="formData.safetySupervision"
              rows="3"
              maxlength="500"
              show-word-limit
            ></el-input>
        ...
        <el-col :span="24" class="grid-cell">
          <el-form-item label="监理工作情况" prop="supervisionWork">
            <el-input
              type="textarea"
              v-model="formData.supervisionWork"
              rows="3"
              maxlength="500"
              show-word-limit
            ></el-input>
        ...
        <el-col :span="24" class="grid-cell">
          <el-form-item
            label="存在的问题需协调解决事项"
            prop="coordinationIssues"
          >
            <el-input
              type="textarea"
              v-model="formData.coordinationIssues"
              rows="3"
              maxlength="500"
              show-word-limit
            ></el-input>
          ...
          <div class="">
            <div class="flex align-center justify-between">
              ...
              <el-button
                :disabled="
                  isAppH5 ||
                  (((tabelData && tabelData.length > 0 && formData.isSigned) ||
                    formData.flowStatus == 2 ||
                    readonly) &&
                    !($route.query.maintenance === '1'))
                "
                type="primary"
                @click="createFile"
                >预览
              ...
              >
                <template slot-scope="{ row }">
                  <el-button
                    ...
                    @click.native.prevent="handlePreview(row, $event)"
                    type="text"
                    size="small"
                  >
                    预览
                  ...
                  <el-button
                    style="margin: 0"
                    @click.native.prevent="handleDownload(row, $event)"
                    type="text"
                    size="small"
                  >
                    下载
                  ...
                  <!-- <ntko
                    ...
                    :disabled="
                      isAppH5 ||
                      row.isSigned ||
                      (!isStampNode && !isStartNode) ||
                      fromAppForm
                    "
                    @h5SginCallBack="h5SginCallBack"
                  ></ntko> -->
                  <el-button
                    @click.native.prevent="handleCancelNtko(row, tableData)"
                    type="text"
                    size="small"
                    :disabled="
                      isAppH5 ||
                  ...
                  <el-button
                    style="margin-left: 0"
                    @click.native.prevent="delFile"
                    type="text"
                    size="small"
                    :style="
                      isAppH5 || !formData.flowStatus || readonly || fromAppForm
                        ? ''
                        : 'color:red'
                    "
                    :disabled="
...
export default {
  ...
  data() {
    return {
      ...
      sectionFixedNames: [
        "左岸场内交通及自然边坡防治工程",
        "八孟公路绿化工程、施工区封闭管理工程、八窝龙乡至八孟公路连接公路工程等施工标",
        "导流洞工程上游施工标",
        "右岸场内交通及自然边坡防治工程施工标",
        "导流洞工程下游施工标",
        "场内交通跨江桥梁及其连接道路工程施工标",
  ...
  methods: {
    ...
    getSectionList() {
      ...
      sectionAllList({}).then((res) => {
        if (res.success) {
          this.sectionFixedOptions = res.data.filter((item) =>
            this.sectionFixedNames.includes(item.name)
          );
        }
      });
    },
