      ...
      <el-row :gutter="20">
        ...
        <el-col :span="24">
          ...
          <el-form ref="dataFormtable">
            <el-table ref="multipleTable" border :data="formData.projectList">
              <el-table-column
                label="序号"
                type="index"
                width="50"
              ...
              >
                <template slot-scope="{ row }">
                  <span v-if="readonly">
                    {{
                      $DictionaryParsing(constructionContentOptions, row.name)
                        ? $DictionaryParsing(
                  ...
                  <el-form-item v-else :error="row.nameError">
                    <el-select @visible-change="$visibleChange($event, 'el-popper')"
                      v-model="row.name"
                      @change="tableParValueChange(row, 'name', 'nameError')"
                      allow-create
                      filterable
              ...
              >
                <template slot-scope="scope">
                  <el-button
                    @click.native.prevent="delProject(scope.$index, tableData)"
                    type="text"
                    size="small"
                    :style="readonly ? '' : 'color: red'"
                    :disabled="readonly"
        ...
        <el-col :span="24">
          ...
          <el-form ref="dataFormtable">
            <el-table ref="multipleTable" border :data="formData.deviceList">
              ...
              <el-table-column label="设备型号" prop="remark" align="center">
                <template slot-scope="{ row }">
              ...
              <el-table-column label="设备名称" prop="remark" align="center">
                <template slot-scope="{ row }">
...
export default {
  ...
  data() {
    return {
      ...
      toNodeData: [],
      currentRow: null, //选中行
      isAdd: true, //是否新增 给发送请求使用 这里监听的是年度切换 过滤到了数据代表这个不是新增 是修改
  ...
  created() {
    this.planType = this.$route.meta.planType;
    this.formData.type = this.planType;
    if (this.planType === 1) {
      this.title = "年度计划";
      this.label = "年度:";
      this.dateType = "year";
  ...
  methods: {
    ...
    getFormData() {
      ...
      this.getFlowRow().then((row) => {
        ...
        if (row && row.subDetail) {
          this.tableData = row.subDetail.sort((a, b) => {
    ...
    save(fields, callback, event) {
      ...
      //subDetail  要进行一个标识符 排序 因为 后端保存  改变了排序
      params.subDetail.forEach((item, index) => {
