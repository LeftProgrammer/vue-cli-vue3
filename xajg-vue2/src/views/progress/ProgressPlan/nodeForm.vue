...
export default {
    ...
    "formData.type": {
      handler(val) {
        if (this.dialogType == "WORK") {
          if (val === "dskw" || val === "dszh" || val === "dscq") {
            this.formDataRules.startStakeMark = [
              {
                required: true,
                message: "请输入",
                trigger: "blur"
            ...
            this.formDataRules.endStakeMark = [
              {
                required: true,
                message: "请输入",
                trigger: "blur"
  ...
  methods: {
    // 计划起桩号
    ruleStartStake() {
      // 移除非数字字符、除第一个小数点以外的小数点以及多余的小数位
      const formattedString = this.formData.startStakeMark
        .replace(/[^\d.]/g, "") // 移除非数字和小数点
        .replace(/^[^\d.]/, "") // 如果开头不是数字，则替换为空
        .replace(/^(\d+\.\d*)\./, "$1") // 保留第一个小数点，替换其他小数点为空
        .replace(/(\.\d{3})\d*$/, "$1"); // 保留三位小数

      this.formData.startStakeMark = formattedString;
    },
    // 计划起桩号
    ruleendStake() {
      // 移除非数字字符、除第一个小数点以外的小数点以及多余的小数位
      const formattedString = this.formData.endStakeMark
        .replace(/[^\d.]/g, "") // 移除非数字和小数点
        .replace(/^[^\d.]/, "") // 如果开头不是数字，则替换为空
        .replace(/^(\d+\.\d*)\./, "$1") // 保留第一个小数点，替换其他小数点为空
        .replace(/(\.\d{3})\d*$/, "$1"); // 保留三位小数

      this.formData.endStakeMark = formattedString;
    },
    //
    closedDialog() {
    ...
    pbsSelectedHandle(pbs) {
      ...
      this.$set(this.formData, "pbsName", pbs.name);
