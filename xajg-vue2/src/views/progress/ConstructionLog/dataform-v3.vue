      ...
      >
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="标段名称" prop="sectionId" align="center">
              ...
              >
                <el-option
                  v-for="item in sectionOptionsSouce"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id"
                />
        ...
        <el-row :gutter="20">
          ...
          <!-- <el-col :span="24">
            <div class="">
              <div class="flex align-center justify-between">
                ...
                <el-button v-if="!readonly" type="primary" @click="addEquipment"
                  >新增
                </el-button>
              </div>
                ...
                >
                  <template slot-scope="{ row }">
                    ...
                    <el-form-item v-else :error="row.nameError">
                      <el-input
                        placeholder="请输入"
                        @blur="tableParValueChange(row, 'name', 'nameError')"
                        v-model="row.name"
                      ></el-input>
                    </el-form-item>
                  </template>
                  ...
                  <template slot-scope="{ row }">
                    ...
                    <el-form-item v-else :error="row.quantityError">
                      <el-input
                        ...
                        @blur="tableParValueChange(row, 'quantity', 'quantityError')"
                      ></el-input>
                    </el-form-item>
                  </template>
                </el-table-column>
          ...
          <!-- <el-col :span="8" style="margin-top: 20px">
            <el-form-item label="生产管理人员数量" prop="managePerson" align="center">
              <el-input
                class="w-100pre"
          ...
          <el-col :span="8" style="margin-top: 20px">
            <el-form-item label="安全管理人员数量" prop="safetyManager" align="center">
              <el-input
                class="w-100pre"
                v-model="formData.safetyManager"
                placeholder="请输入"
                :disabled="readonly"
              >
              </el-input>
          ...
          <el-col :span="8" style="margin-top: 20px">
            <el-form-item label="作业人员数量" prop="worker" align="center">
              <el-input
                class="w-100pre"
...
export default {
  ...
  data() {
    return {
      ...
      formDataRules: {
        ...
        managePerson: [{ required: true, message: '请输入', trigger: 'blur' }],
        safetyManager: [{ required: true, message: '请输入', trigger: 'blur' }],
        constructionUnit: [
          { required: true, message: '请输入', trigger: 'blur' }
        ],
        worker: [{ required: true, message: '请输入', trigger: 'blur' }]
      },
  ...
  computed: {
    ...
    selectCode() {
      this.getSelectCode(this.sectionOptions, this.formData.sectionId)
    },
    userInfo() {
    ...
    flowDisabled() {
      return (field) => {
        let disabled = true
        switch (field) {
          case 'constructionProductionManager':
  ...
  methods: {
    ...
    downTemp() {
      console.log('下载模板')
      let fileName = '施工日报导入模板.xlsx'
      var link = document.createElement('a')
      link.href = '/static/template/施工日报导入模板.xlsx'
    ...
    handleFileUpload(event) {
      ...
      reader.onload = (e) => {
        ...
        const jsonData = XLSX.utils.sheet_to_json(sheet, { raw: false }) //加上{header:1}会把第一行作为表头
        console.log('表格原始数据', jsonData)
        if (jsonData[0]) {
          const sectionData = this.sectionOptionsSouce.find(
            (x) => x.name == jsonData[0]['标段名称']
          )
          Object.assign(this.formData, {
            sectionId: sectionData ? sectionData.id : '',
            sectionName: jsonData[0]['标段名称'],
            temperature: jsonData[0]['气温'],
            workTomorrow: jsonData[0]['明天工作计划'],
            logsDate: new Date(jsonData[0]['日期']),
            constructionSite: jsonData[0]['施工部位'],
            constructionContent: jsonData[0]['施工内容'],
            weather: jsonData[0]['天气'],
            otherContent: jsonData[0]['其他需要说明的情况']
          })
        }
    ...
    async getUnitList() {
      ...
      console.log('请求 单位1 ', this.belongUnit)
    },
    // 获取当前人的所属机构
    async getBelongTo(row) {
      ...
      try {
        ...
        if (Array.isArray(data) && data?.length > 0) {
          ...
          console.log('this.单位2', this.belongUnit)
        } else {
          this.belongUnit = []
        }
      } catch (error) {
    ...
    mergeListToMap(list) {
      ...
      list.forEach((item) => {
        if (map.has(item.type)) {
          map.set(item.type, Number(map.get(item.type)) + Number(item.quantity))
        } else {
      ...
      try {
        ...
        this.$nextTick(async () => {
          ...
          let section = this.sectionOptions.find(
            (item) => item.id === this.formData.sectionId
          )
          this.formData.sectionName = section.name
