  ...
  <div class="section-content-page">
    <el-row :gutter="20">
      <el-col :span="15" class="position-relative">
        <el-date-picker v-model="currentMonth" type="month" @change="changeMonth" value-format="yyyy-MM"
          placeholder="选择月" class="picker-month position-absolute top-12">
        </el-date-picker>

        <el-select @visible-change="$visibleChange($event, 'el-popper')"
          style="position: absolute; left: 430px; top: 12px" v-model="sectionId" placeholder="请选择标段" clearable
          @change="getDefault">
        ...
        <el-calendar v-model="currentDate">
          <template slot="dateCell" slot-scope="{ data }">
            <div v-if="isShowDay(data)" @click="selectDay(data.day, data)"
              ...
              <div style="margin-left: auto" class="current-value">
                <span class="font-30 custom-day mr-10"></span><span class="font-12 font-w-550 mr-10 unit">/份</span>
              </div>
            </div>
          </template>
        </el-calendar>
      ...
      <el-col :span="9" class="content-area">
        ...
        <div class="log">
          <div class="text-center flex" v-if="logLists.length == 0">
            ...
            >111</el-tooltip> -->
            <el-empty description="暂无日志" class="auto w-200 text-center"></el-empty>
          </div>

          <div v-else>
            <div v-for="item in logLists" :key="item.id" class="bg-fff rounded-5 px-10 py-20 mb-20 logItem">
              ...
              <div class="flex align-center mb-15 justify-start font-14">
                ...
                <div class="content-value">
                  {{ getSelectText(sectionOptions, item.sectionId) }}
                </div>
              </div>
              <!-- <div class="flex align-center mb-15 justify-start font-14">
              ...
              <div class="opration flex justify-end">
                ...
                ]" :row="item" :flowName="flowName" @click="handelShowDialog" @delete="deleteLog(item)"></flow-button>
              </div>
            </div>
          </div>
        </div>
...
<script>
import { byMonth, remove, logList, sectionList, page, sectionAllList } from './api-v3';
...
export default {
  ...
  destroyed() {
    localStorage.removeItem("ConstructionLogsDate");
  },
  methods: {
    // 上月 不需要显示
    isShowDay(date) {
    ...
    getSectionList() {
      ...
      sectionList({}).then((res) => {
        if (res.success) {
          ...
          if(res.data){
            console.log("sectionList", res.data[0].id);
            this.sectionId = res.data[0].id
            this.getLogNum()
          }
        }
      });
    ...
    getToday() {
      ...
      if (!constructionLogsDate) {
        constructionLogsDate = new Date();
      }
      this.currentMonth = dateFormat(constructionLogsDate, "yyyy-MM");
      let day = dateFormat(constructionLogsDate, "yyyy-MM-DD");
    ...
    async getLogData() {
      localStorage.setItem("ConstructionLogsDate", this.currentDate);
      // 判断当前日期是不是 标准类型
