  ...
  <div class="page-leave">
        ...
        <div class="flex justify-between">
          ...
          <div>
            <div class="flex align-center">
              <div
                v-if="showSignButton && formData.signStatus == 0"
                class="mr-16"
              >
                <el-button type="primary" @click="setSigned">签收</el-button>
              </div>
              <!-- formData.signStatus == 2 -->
              <div
                v-if="
                  showSignButton &&
                  formData.isMultiple != 2 &&
                  formData.signStatus == 2
                "
                class="mr-16"
              >
                <el-button type="danger" @click="rejectSign(4)">退回</el-button>
              </div>
              <div v-if="showSignButton && formData.signStatus == 0">
                <el-button type="danger" @click="rejectSign(3)">拒签</el-button>
              </div>
              <div
              ...
              >
                <span class="pointer">雄安水电站处理笺 </span>
                <span v-if="fromapp">
                  <span
                    class="ml-20 pointer"
    ...
    <el-row :gutter="20">
      ...
      >
        ...
        <el-col :span="12" v-if="showFileType">
        ...
        <el-col :span="12" v-if="showOnlineEdit">
          <el-form-item label="">
            <el-radio-group
        ...
        <!--1 收文 (只有文件) 2 发文 -->
        <!-- <span v-if="pageType.documentType == 1">
          <el-col :span="12">
        ...
        <span>
          ...
          <!--
             v-if="
              wmNode ? false : formData.sendStatus == 1 || formData.flowStatus
            "
          -->
          <!-- 文秘节点 -->
      ...
      >
        ...
        <el-table-column label="收文单位" prop="sendUnits" align="center">
          <template slot-scope="{ row, $index }">
            <span class="text-ellipsis" :title="getTableUnitName(row, $index)">
...
export default {
  ...
  components: {
    ...
    // RelatedReceivingDocuments: () =>
    //   import(
    //     "@/views/office/SendDocument/components/RelatedReceivingDocuments.vue"
    //   ),
  },
  props: {
    disabledDeep: {
  ...
  computed: {
    ...
    isShowSignTabel() {
      ...
      // 针对pc 端 只有在发出去之后并且有数据的情况下  还有 退回的情况下。
      return (
        this.formDisabled &&
  ...
  methods: {
    ...
    getTableUnitName(row, index) {
      ...
      if (row.unitType == 4) {
        ...
        console.log("a", a);
        if (a) {
          let unitId = a.split("_")[1];
          let depInfo = this.ownDepartmentsByUnit.find(
            (x) => x.departId == unitId
          );
          return depInfo
            ? depInfo.corpName + "-" + depInfo.departName
            : "雄安建设管理局";
        } else {
          return "";
        }

        let depId = row.sendUser.split("_")[1];
    ...
    async setTable_back(row, type) {
      this.$nextTick(async () => {
        this.$refs[type].loading = true;
        let sendUnitArr = row[type]?.split(",") || [];
        let sendUnitTableData = [];
        ...
        for (const unit of sendUnitArr) {
          let forUNIT =
            (type === "sendUnits" ? row.sendUnits : row.copySend)?.split(",") ||
            [];
          for (const val of forUNIT) {
            let isOwnDept = ownDepart.includes(val);
            let params = {
              corpId: isOwnDept ? ownerUnit.corpId : val,
              roleName: "文秘",
            };
            ...
            let sendUser =
              row.sendUser?.split(",").find((x) => x.includes(val)) || "";
            let useId = sendUser.split("_")[0];
            let userItem = secretaryData.find((x) => x.userId === useId);

            if (isOwnDept) {
              secretaryData = secretaryData.filter(
                (item) => item.departId === val
              );
            }

            if (!userItem) {
              const resUserList = await userInfoList({ data: [useId] });
              if (!resUserList.success) {
                this.$message.error("历史数据，人员获取失败");
              }
              if (
                ...
                secretaryData.push(resUserList.data[0]);
              }
            }
    ...
    async preview(cmd) {
      ...
      console.log('formData.uploadFileContent',formData)
    ...
    async getFormData() {
      try {
        ...
        const row = await this.getFlowRow(pageParams);
        if (row) {
          if (row.code) {
