<template>
  <div style="padding: 10px" class="page-leave apph5-parent">
    <el-row>
      <el-col :span="24">
        <div>基本信息</div>
    ...
    >
      ...
      <app-h5-table
        ...
        :tableData="tableData"
        :columns="columns"
        @deleteRowHandle="tableData.splice(index, 1)"
      >
        ...
        <template slot="uploadFile" slot-scope="{ row }">
          <uploadFile
            :fromApplication="appformDisabled ? 'app' : 'web'"
            :class="readonly ? 'custom-disabled' : ''"
            :readonly="readonly"
...
import AppH5Table from "@/components/AppH5/Table/index.vue";
...
export default {
  ...
  methods: {
    ...
    async beforeSend(eventData) {
      if (this.isFormApp) {
        this.$message.warning("生成资金调拨版式文件需前往电脑浏览器端操作");
        return false;
      }
    ...
    formateNtkoData() {
      //标题
      var titleName =
        window.parent.document.querySelector(".title").innerHTML + ".pdf";
    ...
    async getFormData() {
      ...
      console.log("获取表单数据1", JSON.stringify(row));
      if (row) {
