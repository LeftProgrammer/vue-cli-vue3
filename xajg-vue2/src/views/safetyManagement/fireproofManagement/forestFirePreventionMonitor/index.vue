<template>
  <div class="page-list">
        ...
        <div class="position-relative w-100pre h-100pre">
          <div id="mapbox" style="width: 100%; height: 100%; object-fit: fill">
            <div class="bg-map" :style="{ width: `${bgWidth}px` }">
              <img v-for="item in initTreeData" :key="item.id" :src="item.isHighlight
    ...
    <!-- 视频播放 -->
    <el-dialog :title="videoName" custom-class="dialog-box" :before-close="videoClose" :show-close="true" v-draggable
      :visible.sync="show_video" append-to-body :close-on-click-modal="false" destroy-on-close>
      <div class="flex justify-center w-100pre h-600">
        <VideoPlay ref="VideoPlay" class="w-100pre h-100pre" />
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    handleNodeClick(node) {
      ...
      this.treeData[0]?.children?.forEach((item) => {
        this.$set(item, "isHighlight", item.id === node.id);
        if (node.id === item.id) {
          this.pointClick(item);
        }
      });
      // this.url = node.discernNumber
      // this.videoPlay()
      // if (node.id) {
      //   this.treeData[0].children.forEach((item) => {
      //     this.$refs.bimEngine.upDataImg({
      //       id: item.id,
      //       html: this.getHtml(
      //         item.id,
      //         item.deviceStatus === 1,
      //         item.id === node.id
      //       ), // <--- 用户修改这里标签内容，可以是HTML富文本
      //     });
      //   });
      // }
    },
    ...
    async pointClick(item) {
      ...
      if (this.videoType == "RTSP") {
        ...
        if (this.$store.state.videoConfig) {
          ...
          let { host } = this.$store.state.videoConfig;
          if (host) {
            url = url.replace(/(rtsp:\/\/)([^\/]+)(\/)/g, `$1${host}$3`);
            console.log(url);
          }
        }
        item.discernNumber = `${url}?token=${data.token}`;
        console.log("需要换取token", res);
      }
      this.show_video = true;
      this.$nextTick(() => {
        const refs = this.$refs.VideoPlay;
        if (refs) {
    ...
    async loadNode(node, resolve) {
      try {
        ...
        if (res.success) {
          ...
          console.log("this.treeData", this.flattenTreeData);
          this.treeData[0]?.children?.forEach((item) => {
    ...
    async computeVideoUrl() {
      ...
      if (rtspDevice.length > 0) {
        // let channelCodes = "1000034$1$0$3,1000034$1$0$2";
