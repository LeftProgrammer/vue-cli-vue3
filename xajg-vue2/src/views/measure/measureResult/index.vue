      ...
      <template slot="form">
        <el-form :model="searchData" :inline="true">
          ...
          <el-form-item label="成果类型">
            <el-select
              v-model="searchData.achieveType"
              clearable
              placeholder="请选择"
            >
              <el-option
    ...
    >
      ...
      >
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="成果类型" prop="achieveType">
              <el-select
                v-model="dialogFormData.achieveType"
                clearable
                placeholder="请选择"
                style="width: 100%"
                :disabled="dialogReadonly"
          ...
          <el-col :span="12">
            <el-form-item label="报审类型" prop="reportType">
              <el-select
                v-model="dialogFormData.reportType"
                clearable
                placeholder="请选择"
                style="width: 100%"
                :disabled="dialogReadonly"
...
export default {
  ...
  methods: {
    ...
    // 查表格数据
    async getResultPageList() {
      this.tableData = [];
      try {
        const { data, success, message } = await getResultPageList(
          this.searchData
        );
    ...
    async deleteResult(row) {
      try {
        ...
        const { data, success, message } = await deleteResult(row.id);
        if (!success) {
          this.$message.error("测量成果删除失败：" + message);
          return false;
        }
        this.$message.success("测量成果删除成功!");
        this.searchData.pageNum = 1;
        this.getResultPageList();
        return true;
      } catch (error) {
    ...
    handelExport() {
      ...
      window.open(`/api/measure/result/export?${query}`, "_blank");
    },
    beforeUpload(file) {
    ...
    async createResult() {
      try {
        const { data, success, message } = await createResult({
          ...this.dialogFormData,
          attId: this.dialogFormData.fileList.map((item) => item.attId).join(),
        });

        if (!success) {
          this.$message.error("创建测量成果信息失败：" + message);
          return false;
        }
        this.$message.success("创建测量成果信息成功");
        ...
        console.error(err);
        this.$message.error("创建测量成果信息失败");
        return false;
      }
    },
    async updateResult() {
      try {
        const { data, success, message } = await updateResult({
          ...this.dialogFormData,
          attId: this.dialogFormData.fileList.map((item) => item.attId).join(),
        });

        if (!success) {
          this.$message.error("修改测量成果信息失败：" + message);
          return false;
        }
        this.$message.success("修改测量成果信息成功");
        this.handelDialogClose();
        this.getResultPageList();
        return data;
      } catch (err) {
        console.error(err);
        this.$message.error("修改测量成果信息失败");
        return false;
      }
    },
  },
