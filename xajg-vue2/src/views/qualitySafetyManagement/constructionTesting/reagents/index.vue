    ...
    >
      ...
      <template slot="table">
        ...
        >
          ...
          <!-- <el-table-column label="检测报告" width="180" align="center" show-overflow-tooltip prop="uploadFile">
            <template slot-scope="scope">
              <el-button @click="downloadFile(scope.row.uploadFile)" type="text" style="font-size: small;">{{
                scope.row.uploadFile.name }}</el-button>
            </template>
          </el-table-column> -->
          <el-table-column
            label="检测报告"
...
export default {
  ...
  methods: {
    ...
    firstGetTableData(name) {
      //把选择的名称放在缓存里面
      this.$setStorage("materialName", name);
      this.query.name = name;
      this.getTableData();
    },
    getTypeDictName(id) {
