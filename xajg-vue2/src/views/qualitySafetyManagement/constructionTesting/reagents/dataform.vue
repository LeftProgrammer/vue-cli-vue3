...
export default {
  ...
  data() {
    return {
      ...
      formDataRules: {
        ...
        uploadFile: [
          { required: false, message: "请上传报告文件", trigger: "blur" },
        ],
        section: [
  ...
  methods: {
    ...
    getTable() {
      ...
      //   //从缓存里取出材料名称
      //   this.formData.name = this.$getStorage("materialName");
    ...
    submit() {
      ...
      if (this.tableData) {
        ...
        this.formData = { ...this.formData, ...field };
      }
      //修改
      if (!this.enterAble) {
    ...
    closedHandle() {
      ...
      // this.$emit('supervisionName',this.formData.name)
    },
    showPbs() {
