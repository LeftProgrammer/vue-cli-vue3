...
export default {
  ...
  methods: {
    ...
    choose() {
      ...
      if (this.type == 'cover' && this.position) {
        this.coverPos = JSON.parse(this.position)
      }
    },
    //关闭时
    closedHandle() {
    ...
    getpos(pos) {
      ...
      if (this.type == 'point') {
        ...
        var opt = {
          axisType: 1,
          id: 'lablePos',
          position: pos || JSON.parse(this.position),
          displayRange: [0, 100000],
          maxDistance: 100000,
          text: JSON.stringify(pos) || this.position
        }
    ...
    rightClick() {
      if (this.coverPos.length < 3) this.$message.info('至少选中3个点')
        ...
        this.api.Public.drawShape(
          this.coverPos.length > 0 ? this.coverPos : JSON.parse(this.position),
          {
            id: new Date().getTime(),
