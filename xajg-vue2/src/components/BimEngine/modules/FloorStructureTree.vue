...
<script>
import { getFloorStructureTree, getModelTypeTreeIdList } from '@/components/BimEngine/api/document'
export default {
  props: {
    projectMessage: {
      type: Object,
  ...
  methods: {
    ...
    async onCheckTree(checkedKeys, e) {
      ...
      if (checkNode.glid == checkNode.modelId) { //模型级别
        ...
        const res = await getModelTypeTreeIdList({ Pid: e.node.dataRef.glid, LightweightName: e.node.dataRef.modelName })
