...
<script>
import { getSpatialStructureTree, getSpatialStructureIdList } from '@/components/BimEngine/api/document'
export default {
  props: {
    projectMessage: {
      type: Object,
  ...
  methods: {
    ...
    async onCheckTree(checkedKeys, e) {
      ...
      if (checkNode.glid == checkNode.modelId) { //模型级别
        ...
        const res = await getSpatialStructureIdList({ Pid: e.node.dataRef.glid, LightweightName: e.node.dataRef.modelName })
