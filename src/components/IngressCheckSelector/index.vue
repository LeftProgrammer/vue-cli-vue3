<template>
  <el-dialog
    ...
    class="ingress-check-selector-dialog"
    @opened="handleOpened"
    @closed="handleClosed"
  >
...
export default {
  ...
  methods: {
    ...
    async getCurrent() {
      this.current = this.$getStorage && this.$getStorage("userInfo");
      const corpId = this.current && this.current.corpId;
      if (!corpId) return;
      await this.getSectionList(corpId);
    ...
    async getSectionList(corporateId) {
      try {
        ...
        if (!success) {
          this.$message && this.$message.error && this.$message.error("查询标段失败：" + message);
          return false;
        }
        ...
        console.error(err);
        this.$message && this.$message.error && this.$message.error("查询标段失败");
        return false;
      }
    },
    getDefaultSectionId() {
    ...
    getCodeName(row, type) {
      let name = row[type] || (row.designSupply && row.designSupply[type]) || "";
      return name;
    },
