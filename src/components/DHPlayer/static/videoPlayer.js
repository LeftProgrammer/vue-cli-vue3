 ...
 */
 (function () {
    window.dhPlayerControl = Object.assign({
        ...
        DHPlayerVersion: '',
        pkgDHPlayerVerion: [2306060919, 2306060919], // 配套的插件版本号
            ...
            switch (data.method) {
                ...
                case 'video.change.substream':
                    if(channelInfo.byUrl) {
                        return hwndInfo.dhPlayerMessage && hwndInfo.dhPlayerMessage(channelInfo, window.dhPlayerControl.dhMessage[706])
                    }
                    ...
                    hwndInfo.channelList.forEach((item, index) => {
                        if (item.snum === data.info.snum) {
                            ...
                            if(item.recordSource === 2) {
                                // 设备录像要关闭一次视频
                                this.closeVideo(data.info.snum)
                                videoInfo.startPlayback(item, {
                                    isConnect: true,
                                    scaleSteps: Number(data.info.scaleSteps)
                                })
     ...
     */
    function isSupport() {
        let supportedOS = ['Windows 7', 'Windows 10', 'Windows 8', 'Windows 8.1']
        ...
        if (window.dhPlayerControl.isPIframe) {
            ...
            for (var i = 0; i < iframes.length; i++) {
                ...
                if (name) {
                    ...
                    dom = iframes[i].contentWindow.document.getElementById(this.setting.videoId)
                }
                if (dom) {
                    pIframe = iframes[i]
                    break
        ...
        return new Promise((resolve, reject) => {
            ...
            .then((res) => {
                ...
                let records = res.records[param.currentIndex]
                    ...
                    let sTime = param.bBack === 0 ? String(param.currentPlayTime) : String(param.playStartTime)
    ...
    VideoPlayer.fn = VideoPlayer.prototype = {
        ...
        startReal: function (option, {isReOpen, isSubStream, count} = {}, type = 'real') {
            ...
            option.forEach(item => {
                ...
                !isReOpen && !isSubStream && flag && this.closeVideo(item.snum)
                if(!this.setting.request[type]) {
                    this.setting.realError && this.setting.realError(item, {
                        code: 207,
        ...
        realByUrl: function (option, {isSubStream, isReOpen, count} = {}, isProj) {
            ...
            (!isProj || !isReOpen && !isSubStream) && this.closeVideo(option.snum)
            this.send({
         ...
         * @param { Number } option.recordSource 录像类型 2-设备录像 3-中心录像
         * @param { Boolean } option.redirect 重定向，默认false （拼接地址需要改为true，接口返回地址为false）
        ...
        changePosition: function (option, callBack) {
            ...
            if(!this.setting.isPIframe) {
                _info.mozInnerScreenX = (top.mozInnerScreenX * zoom) / 100
                _info.mozInnerScreenY = (top.mozInnerScreenY * zoom) / 100
            }
            delete _info.width
            delete _info.height
            _info.show = document.visibilityState === 'hidden' ? false : true
            _info.title = window.dhPlayerControl.isPIframe ? document.title : window.top.document.title
            let sendPosition = () => {
                this.send(
                    {
