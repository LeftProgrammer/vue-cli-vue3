...
export default {
  ...
  emits: ['update:modelValue', 'update:point', 'change'],
  props: {
    /** Vue3 默认 v-model */
    modelValue: {
      type: String,
  ...
  methods: {
    ...
    surePoint() {
      ...
      this.$emit("update:modelValue", this.pointPop);
      this.$emit("update:point", this.pointPop);
  ...
  computed: {
    effectivePoint() {
      return this.modelValue !== undefined && this.modelValue !== null && this.modelValue !== "" ? this.modelValue : this.point;
    }
  },
  watch: {
    effectivePoint: {
