...
export default {
  ...
  methods: {
    ...
    sliderChangeHandle(newValue, oldValue) {
      ...
      if (newValue > oldValue) {
        ...
        if (newValue < this.hideval) {
          for (let i = newValue; i < this.guidsAry.length; i++) {
    ...
    async InitEngine() {
      ...
      if (!API) {
        that.$message && that.$message.error("BIM API 未加载");
        return;
      }
      ...
      const docId = that.$route.query.modelId
        ...
        : that.id;
      if (docId) {
        const resDoc = await getDocById(docId);
        const config = await getDocumentConfig({ DocumentId: docId });
        if (config.items.length) that.DocConfig = config.items[0];
        console.log(that.DocConfig);
        that.modelList.push({
          name: resDoc.modelName,
          id: resDoc.id,
          docName: resDoc.docName,
          suffix: resDoc.suffix,
          docType: resDoc.docType,
          checked: true,
        });
        that.projectMessage = resDoc;
        ...
        const combineId = that.$route.query.combineId
          ...
          : that.cid;
        const resCombine = await getClampById(combineId);
        if (resCombine.combineDetails.length == 0) return;
        for (let i = 0; i < resCombine.combineDetails.length; i++) {
          that.modelList.push({
            name: resCombine.combineDetails[i].document.modelName,
            id: resCombine.combineDetails[i].document.id,
            docName: resCombine.combineDetails[i].document.docName,
            suffix: resCombine.combineDetails[i].document.suffix,
            docType: resCombine.combineDetails[i].document.docType,
            matrix: JSON.parse(resCombine.combineDetails[i].matrix),
            checked: true,
          });
          that.projectMessage = resCombine;
          that.projectMessage.modelList = that.modelList;
          that.projectMessage.sceneType = 0;
