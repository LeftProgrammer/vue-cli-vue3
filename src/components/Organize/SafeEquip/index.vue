<template>
  <div
    ...
    :class="{
      readonly: readonly || disabled || disabledProp || innerDisabled,
    }"
  >
    <div class="choose-dev">
      <div class="tags text" v-if="equipNames.length == 0">请选择</div>
      <div class="tags" v-else>
        <el-tag v-for="(name, i) in equipNames" :key="i" type="info">
          {{ name }}
        </el-tag>
      </div>
    ...
    >
      <div class="staff-main" :class="{ single: !multiple }">
        <div class="staff-table">
          ...
          <div style="height: 400px; overflow-y: auto;">
            ...
            >
              ...
              <el-table-column prop="name" width="" label="设备名称" align="left">
              </el-table-column>
              <el-table-column
                prop="code"
                width="180"
        ...
        <div class="staff-choosed">
          <div class="total">已选（{{ tempSelectedEquips.length }}）</div>
          <div class="choose-staff">
            <div
              v-for="(staff, i) in tempSelectedEquips"
              :key="staff.id"
            ...
            >
              <el-tag
                size="medium"
                effect="dark"
                @close="() => delChoosedEquips(staff, i)"
                closable
              >
                {{ staff.name }}
              </el-tag>
...
import {
  page,
  BelongTo,
} from "@/views/safetyManagement/equipmentManagement/api";

export default {
  name: "Organize-SafeEquip-Index",
  emits: ["update:modelValue", "update:value", "change", "closed"],
  props: {
    /** Vue3 默认 v-model */
    modelValue: {
      type: [String, Array],
  ...
  data() {
    return {
      ...
      /** 已选择的人员列表 */
      choosedEquips: [],
      /** 临时存储对话框中的选择，只有确认后才会更新到 choosedEquips */
      tempSelectedEquips: [],
      /** 表格数据 */
      tableData: [],
      /** 总记录数 */
      ...
      /** 当前选中单位的名称 */
      unitName: "",
      /** 控件是否禁用（内部状态，避免与 props.disabled 重名） */
      innerDisabled: false,
      /** 来自props的禁用状态 */
      disabledProp: false,
      /** 表单实例 */
  ...
  computed: {
    effectiveValue() {
      return this.modelValue !== undefined && this.modelValue !== null && this.modelValue !== "" ? this.modelValue : this.value;
    },
    /**
     * 设备名称数组
     */
    equipNames() {
      console.log("equipName,", this.choosedEquips);
      // 直接从 choosedEquips 中提取名称，这样可以确保显示的名称与实际选中的项完全一致
      return this.choosedEquips.map((item) => item.name);
    },
  },
  mounted() {
    this.getIsDisabled();
  ...
  methods: {
     ...
     */
    choose() {
      if (this.readonly || this.disabled || this.disabledProp || this.innerDisabled) {
        return;
      }
      // 打开对话框时，将已选人员复制到临时选择中
      this.tempSelectedEquips = [...this.choosedEquips];
        ...
        .then((res) => {
          ...
          this.$nextTick(() => {
            if (
              ...
              this.tempSelectedEquips.length > 0
            ) {
              // 首先清除所有选中状态
              this.$refs.staffTable.clearSelection();

              // 然后选中已选项
              this.tempSelectedEquips.forEach((staff) => {
     ...
     * 删除已选择的设备
     */
    delChoosedEquips(staff, index) {
      // 如果在对话框中，删除临时选择
      if (this.dialogShow) {
        this.tempSelectedEquips.splice(index, 1);
        // 移除表格中的选中状态
        ...
        // 否则删除确认选择并更新模型
        this.choosedEquips.splice(index, 1);
        this.updateModel();
      }
    },
    ...
    handleSelectionChange(selection) {
      if (this.multiple) {
        // 将选中的行更新到临时选择中
        this.tempSelectedEquips = [...selection];
      }
    },
    ...
    confirmHandle() {
      console.log(
        "确认选择前的临时选择：",
        JSON.stringify(this.tempSelectedEquips)
      );
      // 将临时选择更新到确认选择
      this.choosedEquips = [...this.tempSelectedEquips];
      // 更新模型值
      console.log(
        "更新模型前的choosedEquips：",
        JSON.stringify(this.choosedEquips)
      );
      this.updateModel();
      console.log(
        "更新模型后的choosedEquips：",
     ...
     */
    updateModel() {
      console.log("this.choosedEquips", this.choosedEquips);
      if (this.multiple) {
        // 多选模式返回ID数组
        const ids = this.choosedEquips.map((item) => item.id);
        this.$emit("update:modelValue", ids);
        this.$emit("update:value", ids);
        this.$emit("change", ids);
        this.equipName = this.choosedEquips.map((item) => item.name).join(",");
        console.log(ids, this.equipName);
      } else {
        // 单选模式返回单个ID
        const id =
          this.choosedEquips.length > 0 ? this.choosedEquips[0].id : "";
        this.$emit("update:modelValue", id);
        this.$emit("update:value", id);
        this.$emit("change", id);
        this.equipName =
          this.choosedEquips.length > 0 ? this.choosedEquips[0].name : "";
      }
    },
      ...
      page(params)
        .then((res) => {
          ...
          if (filteredRecords.length > 0) {
            if (this.multiple) {
              // 多选模式，保留所有匹配的记录
              this.choosedEquips = filteredRecords;
            } else {
              // 单选模式，只保留第一条记录
              this.choosedEquips = [filteredRecords[0]];
            }
            // 更新显示的名称
            this.equipName = this.choosedEquips
              .map((item) => item.name)
              .join(",");
    ...
    modelValue: {
      handler(newValue) {
        if (this.effectiveValue) {
          this.getCurrentStaff(this.effectiveValue);
        } else {
          this.choosedEquips = [];
          this.tempSelectedEquips = [];
          this.equipName = "";
